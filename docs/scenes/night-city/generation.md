...
"최대 44.1m 겹친다" 고 신고했다 (동결 전 일지 기록 — 일지는 폐기됨).
...
| | |
|---|---|
| 구역차 | `TYPES` 는 아직 **전역 목록**이다. 다만 실측상 `buildBay` 에 도달하는 구역이 상업뿐이라 지금은 차이가 안 난다 (`towers.podium` 은 죽은 경로다 — status.md "알려진 한계") |
...
10. **텍스처의 행·열 수는 "한 장이 무엇을 덮는가" 와 맞아야 한다.**
    ...
    한 층에 창이 세 줄로 들어가 창이 잘아 보였다 (동결 전 일지 기록)
