...
`shopfront.js` 가 이 도시의 점포 체계다.
    ...
    ALCOVE 1.3   벽감 깊이
    SHOP_H 3.4   1층 층고
    CANOPY_D 1.7 출입구 차양이 벽 밖으로 나오는 깊이 — **유일한 출처**
    frameOf(sub, side)   면 위 좌표계 — at(u,d) · rect(u0,u1,d0,d1) · dims(w,d)
    buildBay(b, sub, side, y, rng, mats, D, signs)

    TYPES 일곱
      열린가게 2.2 · 편의점 1.6 · 노점 1.5 · 바문 1.4 · 국숫집 1.3
      리퍼닥 0.9 · 드롭포인트 0.7
    보조: shuttered · awning · sideReturns · entranceBay · showcase
    표본: `__shops()` — 가중치 낮은 유형이 한 번도 안 나오는 것을 잡는다

`국숫집` 은 카운터·스툴·매달린 전구·김 나는 냄비를 만들고, 스툴 55% 에 앉은
사람이 있다.

**소비자**: `bazaar.shopStrip` → `buildBay`. 시장 대문과 아케이드는 자기
점포 띠를 따로 만든다. 기업 기단은 `showcase` 를, 주거 슬래브는 `entranceBay`
를 직접 쓴다.

### 알코브 안에는 어두운 유리를 두지 않는다

이 씬에서 **발광면은 주변을 밝히지 않는다.** 알코브 안 소품은 밝은 배경판
앞의 **실루엣으로만** 읽힌다. 그래서 규칙이 둘이다.

1. 알코브마다 **발광 배경판을 먼저** 세운다
2. 그 앞에 **어두운 유리를 두지 않는다** — 멀리온만 남긴다

같은 결함이 세 곳에 있었다 (편의점 · `showcase` · 기업 기단 로비 유리).

### 남은 한계

| | |
|---|---|
| 구역차 | `TYPES` 는 아직 **전역 목록**이다. 다만 실측상 `buildBay` 에 도달하는 구역이 상업뿐이라 지금은 차이가 안 난다 (`towers.podium` 은 죽은 경로다 — status.md 3.19) |
| 야간 가독성 | 리퍼닥은 밤 4m 에서 아직 읽기 어렵다 |
...
8. **검사를 새로 넣었으면 일부러 한 번 틀리게 만들어 빨간불을 본다.**
   ...
   **판정이 걸릴 수 없는 모양이어서**였다 (../../lessons.md 2.4)
