...
5. 재질은 `new THREE.Material` 로 직접 만들지 않고 마스터에서 `instance()` 로
   뽑는다 — 값이 같으면 공유되고, 파라미터 이름 오타가 즉시 잡힌다
6. (선택) 감사 하한·비율은 `meta.audit = { expect, ratios }` 로, 씬 전용 진단
   집계는 `createWorld` 반환의 `debug: { 이름: fn }` 으로 신고한다 — 하네스가
   `__<이름>` 콘솔 명령으로 노출한다 (architecture.md 의 계층 정리)

**모듈 수준 상태를 새로 만들면 `onSceneReset()` 에 등록한다** (아래 2절).
안 하면 예외가 아니라 이전 씬 잔재가 조용히 섞인다.
(이 목록이 유일본이다 — README 는 여기를 가리킨다, 2026-08-08.)
