...
// 원형 구덩이의 벽·나선 램프·나선 계단처럼 **한 바퀴 돌아가며 앉는 조각**은
// 회전이 필요하다.
//
// 조각마다 손으로 삼각함수를 쓰면 반드시 어딘가 틀린다 — 이 프로젝트에서
// 이미 두 번 틀렸다 (slum.js 머리말). 그래서 하나만 둔다.
//
// ── 축이 어디로 가는지 (한 번 틀렸다) ─────────────────────────────────────
...
// 살이 되어 바깥으로 삐죽 뻗고 벽 사이가 전부 뚫린다.
// 즉 **w 는 반지름 방향 두께, d 는 호 길이**다.
//
// landmark 와 factory 가 글자 하나 안 틀리고 같은 것을 각자 갖고 있었다
// (코드리뷰에서 확인). 하나로 모은다.
export function yawBox(w, h, d, at, yaw) {
  const g = new THREE.BoxGeometry(w, h, d);
  g.rotateY(yaw);
