...
const EXPECT = {
  ...
  사람: { min: 1000 },
  가로시설: { min: 100 },
  // 오피스 섹터 — 상호작용 소품(개별 노드). 실측 220 (2026-08-05 층 축소 후
  // — 플라자 절반·방 재편으로 353 에서 내려왔다). 분리 로직이 조용히 죽어
  // 전부 병합으로 돌아가면 여기서 잡힌다.
  interactables: { min: 160 },
  // 오피스 섹터 — 약탈 열림 포즈 (씬 밖 그룹, 익스포트 전용). 실측 137
  // (층 축소 후). spawnPair 가 조용히 spawn 으로 퇴화하면 여기서 잡힌다.
  lootPoses: { min: 100 },
  // 오피스 섹터 — 탈출 덕트 그릴 (스토리의 오프닝 동선). 시작 방 입구와
  // 화장실 출구, 최소 둘. 없어지면 탈출 루트가 통째로 사라진 것이다.
  vent: { min: 2 },
  // 오피스 섹터 — 잔소품 합계 (컵·트레이·접시더미·통조림·비누·시계·쓰레기통).
  // 종류마다 하한을 달면 경보가 여섯 줄이 되므로 **한 계통으로 신고**한다
  // (props.createProps). 클러터 패스가 통째로 죽으면 여기서 잡힌다.
  clutter: { min: 40 },
};
