...
const BUDGET = {
  ...
  //
  // ── 4.0M -> 8.0M (사용자 지시) ───────────────────────────────────────────
  // *"마지막 단계에서는 트라이앵글 수가 늘어도 좋으니까, 여러 오브젝트들에
  // 대해서 지오메트리 퀄리티 증가시키는 방안도 진행할거야"*
  //
  // **브라우저는 확인용이지 최종 산출물이 아니다.** 최적화는 유니티에서 하고,
  // 여기서 삼각형을 아끼는 것은 형태를 나쁘게 만드는 대가만 치른다. 실제로
  // 공정탑·나선 계단을 넣자마자 이 선에 걸렸는데, 그건 형태가 좋아진 것이지
  // 낭비가 생긴 것이 아니다.
  //
  // 그래도 **선은 남긴다** — 없애면 폭주(같은 것을 두 번 그림 · 밀도 오타)를
  // 못 잡는다. 지금의 두 배인 8.0M 으로 올린다. 여기 걸리면 그때는 정말로
  // 무언가 잘못된 것이다. 진짜 제약은 여전히 텍스처 220MB 하나다.
  triangles: 8_000_000,
  textureMB: 220,
  // 900 (도시 실측 469) -> 1800: 오피스 섹터가 상호작용 소품 353개를 개별
  // 노드로 분리하면서 1473 이 됐다 (2026-08-05). 실측 3.9ms/frame (절두체
  // 컬링 후 드로우콜 699) 이라 비용은 감당 범위다. 병합을 푼 것은 게임 쪽
  // 요구(개별 콜라이더·크랙·약탈 포즈) 때문이고, 그 사정은
  // scenes/office-sector/props.js 의 interactSet 머리말에 있다.
  drawGroups: 1800,
  programs: 60,
...
// ── 하한과 비율은 씬이 신고한다 (meta.audit) ───────────────────────────────
//
// 여기는 **형식만** 안다 — "이 이름의 개수가 이 하한 밑이면 경고" 와
// "a/b 가 이 대역 밖이면 경고". 어떤 이름에 어떤 값이 맞는지는 그 씬의
// 의미론이라 core 에 둘 수 없다 ("core 는 씬을 모른다" — scenes/index.js).
// 실제로 간판·슬럼·interactables 하한이 여기 하드코딩돼 있었다 (2026-08-08
// 계층 정리로 각 씬의 meta.audit 로 옮겼다 — 숫자의 내력도 그쪽에 있다).
//
//   meta.audit = {
//     expect: { 이름: { min } },                       // "사라졌다" 를 잡는다
//     ratios: [{ name, a, b, min, max }],              // "절반이 죽었다" 를 잡는다
//   }
//
// 하한만으로는 절반이 죽은 것을 못 잡는다 — 간판이 1,602 에서 900 으로
// 죽어도 하한 200 은 아무 말도 안 한다. 그래서 비(比)를 함께 본다: 규모가
// 변해도 건물 한 채가 다는 간판 수는 크게 안 변해야 하고, 변했다면 규모가
// 아니라 **관계**가 깨진 것이다.
...
export function auditScene({ scene, renderer, stats = {}, spec = {} }) {
  const notes = [];
  ...
  for (const m of materials.values()) {
    ...
    //     구워야 한다 (meshkit.scaleUV / metricBox). 맵 목록은 meshkit 의
    //     REPEAT_MAPS 와 같은 6종이어야 한다 — 4종만 보다가 alphaMap·aoMap 에
    //     남은 repeat 을 못 잡는 구멍이 있었다 (2026-08-08 코드리뷰).
    for (const k of ['map', 'normalMap', 'roughnessMap', 'emissiveMap', 'alphaMap', 'aoMap']) {
      const t = m[k];
      if (t && (t.repeat.x !== 1 || t.repeat.y !== 1)) {
  ...
  // 무엇이 얼마나 있어야 하는지는 씬이 신고했다 (파일 머리의 meta.audit).
  const counts = stats.counts || {};
  for (const [name, { min }] of Object.entries(spec?.expect || {})) {
