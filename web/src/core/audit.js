...
const EXPECT = {
  ...
  사람: { min: 1000 },
  가로시설: { min: 100 },
  // 오피스 섹터 — 상호작용 소품(개별 노드). 실측 353 (2026-08-05). 분리
  // 로직이 조용히 죽어 전부 병합으로 돌아가면 여기서 잡힌다.
  interactables: { min: 250 },
  // 오피스 섹터 — 약탈 열림 포즈 (씬 밖 그룹, 익스포트 전용). 실측 229.
  // spawnPair 가 조용히 spawn 으로 퇴화하면 여기서 잡힌다.
  lootPoses: { min: 150 },
};
