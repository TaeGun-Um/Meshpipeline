...
function tubeLamp(b, x, z, rot, y, mats, pools, dead) {
  ...
  // ── 벽 웅덩이를 뺐다 (사용자 지시) ──────────────────────────────────────
  // "등 밑에 저 직각으로 있는 불이 맞닿는 효과 없에"
  //
  // 벽 웅덩이는 폭·높이를 가진 **사각 판**이라 아래 끝이 바닥에서 딱 잘린다.
  // 바닥 웅덩이와 만나는 자리에 직각 이음매가 생기고, 그러면 빛이 아니라
  // 벽에 붙인 스티커로 보인다. 좁은 골목에서는 그 이음매가 늘 눈높이에 있다.
  //
  // 바닥 웅덩이만 남긴다 — 그건 원형 감쇠라 끝이 안 보인다.
}
