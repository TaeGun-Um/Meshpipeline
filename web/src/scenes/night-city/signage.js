...
// ── 배색은 생산자가 고르되, **몇 가지인지는 여기만 안다** ──────────────────
//
// (코드리뷰에서 나온 것)
//
// 생산자 여섯이 `scheme: rng.int(0, 5)` 라고 **열세 곳**에 적고 있었다.
// `SIGN_SCHEMES.length - 1` 을 열세 곳이 각자 아는 것이고, 그게 결합 오류다.
//
// 이 세션에 실제로 터졌다 — `billboard` 배색을 6 -> 4 로 줄이자 `streetlife`
// 와 `program` 이 없는 색인을 집어 빌드가 죽었다. 지금은 `signMat` 의
// `% schemes` 가 흡수하지만, **흡수한다는 것은 분포가 조용히 치우친다는 뜻**
// 이기도 하다 (배색이 셋인 종류에 0..5 를 접으면 0·1·2 가 두 배씩 나온다).
//
// 난수 소비는 똑같이 int 한 번이라 픽셀이 안 변한다.
export function pickScheme(rng) {
  return rng.int(0, SIGN_SCHEMES.length - 1);
}
