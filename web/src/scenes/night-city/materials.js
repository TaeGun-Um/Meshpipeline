...
export function buildMaterials() {
  ...
  return {
    ...
    // 없고, 오히려 어두운 단색 하나가 군중으로 보이는 데 유리하다.
    // 개성은 색이 아니라 체형 셋과 발광 트림으로 낸다.
    //
    // 다만 완전히 검으면 배경과 붙어 사라진다. 환경 반사를 올려서
    // 네온 아래를 지날 때 가장자리가 살아나게 한다.
    personMat: SolidSurface.instance(
      { color: 0x1c1e26, roughness: 0.72, envMapIntensity: 2.4 }, 'Person'
    ),
