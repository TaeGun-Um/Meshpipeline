...
const SEEDS = [
  ...
  //
  // **씨앗 하나다** (사용자 지시: "얘네도 한쪽 구역으로 몰아서 단지로").
  // 전에는 0.90/0.84 짜리 둘이라 도시 양쪽에 골조가 흩어졌고, 그러면
  // "기업이 한 덩어리로 사 모았다" 는 내력과도 어긋난다 — 사 모았으면
  // 한 군데여야 한다. 넓이를 유지하려고 pull 을 올렸다 (보로노이 넓이는
  // 대략 pull 의 제곱에 비례하므로 sqrt(0.90^2 + 0.84^2) = 1.23).
  { ix: 3, iz: 4, kind: 'slum', pull: 1.24 },
