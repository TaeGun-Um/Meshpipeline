...
// (docs/lessons.md 2.1). 검사가 그 실수를 다시 저지르면 검사를 못 믿는다.
//
...
function roadIntrusion(box, bands, axis) {
  // 보행 전용 범위 안은 차도가 아니다. `roads()` 는 1차원 띠 목록이라 "이 구간만 닫혔다"
  // 를 표현할 수 없으므로 (parcel.js 머리말과 같은 사정) 여기서 걸러 준다.
  // 이게 없으면 광장·쉼터의 화분·가로수·기둥이 전부 '도로 침범' 으로 신고된다.
  if (precinctHits(box)) return 0;
  let worst = 0;
...
export function checkPlacement(scene) {
  ...
  // ── 데크에 올라갈 계단이 있나 ────────────────────────────────────────────
  //
  // 당시 일지 1.3 "올라갈 수 있는 곳이 실제로는 없다" (일지는 폐기됨). 계단이 데크와 실제로
