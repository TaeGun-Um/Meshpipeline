...
class NightCity extends Scene {
  constructor() {
    super({
      ...
      // ── 감사 하한·비율 (core/audit.js 가 형식만 검사한다) ─────────────────
      //
      // 숫자의 내력은 core/audit.js 에 있던 것을 그대로 옮겼다 (2026-08-08
      // 계층 정리 — 이 씬의 의미론이므로 이 씬이 신고한다).
      //
      // 하한은 상한이 아니라 "사라졌다" 를 잡는다. 넉넉히 잡는다 — 정확한
      // 값을 박으면 의도한 변경마다 경고가 떠서 아무도 안 보게 된다.
      // 경고가 떴다고 하한을 내리면 표가 아무것도 안 지킨다. 내려도 되는
      // 경우는 **의도한 변경이 개수를 바꿨고 비율이 전부 정상일 때**뿐이다.
      // (건물 300 -> 200: 필지 확대·대지 병합으로 316 -> 242, 비율 넷 정상
      // 확인 후 내렸다. 골목은 사용자 지시로 폐기해 항목을 뺐다.)
      //
      // 비율은 "절반이 죽은 것" 을 잡는다 — 절대 개수로는 "도시가 커졌다" 와
      // "결합이 깨졌다" 를 구분할 수 없다. 대역은 실측의 0.75~1.6배.
      // '길에 면한 면' 만 하한을 규칙(0.39)보다 낮은 0.30 으로 둔다 —
      // 슈퍼블록 병합에서는 안쪽 필지가 길에 안 면하는 것이 정상이라 자연히
      // 내려가기 때문이다. 0.30 아래면 병합이 아니라 고장이다 (streetFaces 의
      // 직접 계측 — 이 비율이 개수보다 먼저 떨어진다).
      // '건물당 간판' 3.5~7.5 는 대지 병합 후 재실측(건물 252 · 간판 1,187 =
      // 4.71)의 0.75~1.6배다 — 옛 대역(2.1~4.4)은 정상 상태에서 항상 켜져
      // 있었고, 늘 켜진 경보는 아무도 안 본다.
      audit: {
        expect: {
          간판: { min: 200 },
          건물: { min: 200 },
  ...
  async createWorld({ scene, renderer, rng, step, camera }) {
    ...
    return {
      built,
      placement,
      // 진단 집계 — 하네스가 `__<이름>` 으로 노출한다 (harness.js). 유형
      // 분포는 "가중치 낮은 유형이 한 번도 안 나오는 것" 을 잡는 용도라
      // 씬이 활성일 때만 뜻이 있다 — 그래서 main 이 아니라 씬이 신고한다.
      debug: {
        shops: async () => (await import('./shopfront.js')).shopTally(),
        houses: async () => (await import('./housing.js')).houseTally(),
        slum: async () => (await import('./slum.js')).slumTally(),
        factory: async () => (await import('./factory.js')).factoryTally(),
      },
